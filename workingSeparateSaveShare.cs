...
using pmjo.NextGenRecorder.Sharing; // uncommented
    ...
    public class SimpleRecorder : MonoBehaviour
    {
        ...
        void RecordingExported(long sessionId, string path, Recorder.ErrorCode errorCode)
        {
            if (errorCode == Recorder.ErrorCode.NoError)
            {
                ...
                // FIGURE OUT HOW TO MAKE IF / ELSE STATEMENTS TO HAVE DIFFERENT OUTCOMES --> all connected to same buttons

                // get current color - put in other script
                Vector4 stateButtonColor = stateButton.image.color;

                if (stateButtonColor[0].Equals((float)0.1))
                {
                    // Do nothing
                }
                else if (stateButtonColor[0].Equals((float)0.2))
                {
                    Sharing.SaveToPhotos(path, "Vertical Jumps"); // immediately save to photos
                }
                else if (stateButtonColor[0].Equals((float)0.3))
                {
                    PlayVideo(path); // view replay recording
                }
                else if (stateButtonColor[0].Equals((float)0.4))
                {
                    Sharing.ShowShareSheet(path, true); // share with others
                }
#endif
